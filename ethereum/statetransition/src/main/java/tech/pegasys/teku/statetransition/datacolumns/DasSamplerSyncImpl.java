...
public class DasSamplerSyncImpl implements SlotEventsChannel, DasSamplerSync {
  private static final Logger LOG = LogManager.getLogger("das-nyota");

  private final DataColumnSidecarRetriever retriever;
  private final DataColumnSidecarSampler sampler;
  ...
  public DasSamplerSyncImpl(
      DataColumnSidecarSampler sampler,
  ...
  public DasSamplerSyncImpl(
      DataColumnSidecarSampler sampler, DataColumnSidecarRetriever retriever) {
    this(sampler, retriever, 10 * 1024, 2 * 1024);
  }

  private synchronized void onRequestComplete(PendingRequest request, DataColumnSidecar response) {
    sampler.onNewValidatedDataColumnSidecar(response);
